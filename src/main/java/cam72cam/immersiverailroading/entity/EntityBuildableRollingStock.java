...
public class EntityBuildableRollingStock extends EntityRollingStock {
		...
		for (int i = 0; i < player.inventory.getSizeInventory(); i ++) {
			...
			if (found.getItem() == IRItems.ITEM_ROLLING_STOCK_COMPONENT) {
				if (ItemDefinition.getID(found).equals(this.defID)) {
					if ((player.capabilities.isCreativeMode || ItemGauge.get(found) == this.gauge) && !ItemRollingStockComponent.requiresHammering(found)) {
	...
	public boolean interactFirst(EntityPlayer player) {
		...
		if (player.getHeldItem().getItem() == IRItems.ITEM_LARGE_WRENCH || player.getHeldItem().getItem() == IRItems.ITEM_ROLLING_STOCK_COMPONENT) {
