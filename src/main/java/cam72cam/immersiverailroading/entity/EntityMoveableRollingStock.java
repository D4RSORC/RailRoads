	...
	public void onUpdate() {
	    ...
	    if (this.getCurrentSpeed().metric() > 1) {
			...
			for (Entity entity : entitiesWithin) {
				...
				if (entity.ridingEntity instanceof EntityMoveableRollingStock) {
				...
				if (!this.getCollisionBoundingBox().intersectsWith(entity.getBoundingBox())) {
					// miss
					continue;
				}
			...
			for (Entity entity : entitiesAbove) {
				...
				// Chunk.getEntitiesOfTypeWithinAABB() does a reverse aabb intersect
				// We need to do a forward lookup
				if (!bb.intersectsWith(entity.getBoundingBox())) {
					// miss
					continue;
				}
