...
public class ClientProxy extends CommonProxy {
	...
	public static void dampenSound() {
		...
		if (player != null && player.isRiding() && player.ridingEntity instanceof EntityRidableRollingStock) {
			EntityRidableRollingStock ridableStock = (EntityRidableRollingStock) player.ridingEntity;
	...
	public static void onKeyInput(ClientTickEvent event) {
		...
		EntityRidableRollingStock riding = (EntityRidableRollingStock) player.ridingEntity;
	...
	public static void onClick(MouseEvent event) {
		...
		if ((event.button == attackID || event.button == useID) && event.buttonstate) {
			...
			int button = attackID == event.button ? 0 : 1;
	...
	public static void onSoundLoad(SoundLoadEvent event) {
		if (manager == null) {
			manager = new IRSoundManager(event.manager);
		} else {
	...
	public static void configChanged(OnConfigChangedEvent event) {
		if (event.modID.equals(ImmersiveRailroading.MODID)) {
