...
public class TileRail extends TileRailBase {
	...
	public net.minecraft.util.AxisAlignedBB getRenderBoundingBox() {
		...
		return AxisAlignedBB.getBoundingBox(-length, -length, -length, length, length, length).offset(pos);
	}
	...
	public void nextTablePos(boolean back) {
		...
		List<EntityCoupleableRollingStock> ents = world.getEntitiesWithinAABB(EntityCoupleableRollingStock.class, AxisAlignedBB.getBoundingBox(-info.settings.length, 0, -info.settings.length, info.settings.length, 5, info.settings.length).offset(this.getPos()));
	...
	public void readFromNBT(NBTTagCompound nbt) {
		...
		if (nbt.hasKey("info")) {
            ...
            placementInfo = new PlacementInfo(placementInfo.placementPosition, placementInfo.direction, placementInfo.yaw, 0);
