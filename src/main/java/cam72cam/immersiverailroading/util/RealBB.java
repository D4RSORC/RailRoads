...
public class RealBB extends AxisAlignedBB {
	...
	private static double[] constructorParams(double front, double rear, double width, double height, float yaw, double centerX, double centerY, double centerZ) {
		...
		AxisAlignedBB newthis = rightBox.func_111270_a(leftBox).offset(centerX, centerY, centerZ);
	...
	public Pair<Boolean, Double> intersectsAt(double minX, double minY, double minZ, double maxX, double maxY, double maxZ, boolean useHeightmap) {
		if (!super.intersectsWith(super.getBoundingBox(minX, minY, minZ, maxX, maxY, maxZ))) {
	...
	public MovingObjectPosition calculateIntercept(Vec3 vecA, Vec3 vecB) {
		...
		for (int step = 0; step < steps; step ++) {
			Vec3 stepPos = Vec3.createVectorHelper(vecA.xCoord + xDelta * step, vecA.yCoord + yDelta * step, vecA.zCoord + zDelta * step);
			if (this.contains(stepPos)) {
				return new MovingObjectPosition(0,0,0,EnumFacing.UP.getIndex(),stepPos); 
			}
		}
		return null;
	}
