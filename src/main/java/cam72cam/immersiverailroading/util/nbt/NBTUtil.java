package cam72cam.immersiverailroading.util.nbt;
...
public final class NBTUtil
{
    ...
    public static GameProfile readGameProfileFromNBT(NBTTagCompound compound)
    {
        ...
        try
        {
            ...
            if (compound.hasKey("Properties", 10))
            {
                ...
                for (Object ss2 : nbttagcompound.func_150296_c())
                {
    ...
    public static boolean areNBTEquals(NBTBase nbt1, NBTBase nbt2, boolean compareTagList)
    {
        ...
        else if (nbt1 instanceof NBTTagCompound)
        {
            ...
            for (Object ss : nbttagcompound.func_150296_c())
            {
            	String s = (String)ss;
        ...
        else if (nbt1 instanceof NBTTagList && compareTagList)
        {
            ...
            else
            {
                for (int i = 0; i < nbttaglist.tagCount(); ++i)
                {
                    NBTBase nbtbase = nbttaglist.getCompoundTagAt(i);
                    boolean flag = false;

                    for (int j = 0; j < nbttaglist1.tagCount(); ++j)
                    {
                        if (areNBTEquals(nbtbase, nbttaglist1.getCompoundTagAt(j), compareTagList))
                        {
                            flag = true;
                            break;
                        }
